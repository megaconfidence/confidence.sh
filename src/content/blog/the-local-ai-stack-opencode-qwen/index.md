...
Qwen 3.6’s model weights are yet to be released (may be out by the end of the week), so this guide will use Qwen 3.5, which is available now for download. Let’s dive in.

## Step 1: Choosing the right Qwen model
...
I’d like to share two more resources with you. To run multiple models simultaneously, check out [llama-swap](https://github.com/mostlygeek/llama-swap). Also, if you have an Apple M-series Mac, consider switching from Llama.cpp to [MLX-LM](https://huggingface.co/mlx-community) for a 20-40% performance boost.
